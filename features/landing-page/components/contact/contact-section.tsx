...
export function ContactSection() {
  ...
  const [visValues, setVisValues] = useState<number[]>([10, 10, 10, 10, 10, 10, 10, 10])
  ...
  // ─── Transport Time UI & Visualizer ──────────────────────────────
  useEffect(() => {
    if (!isPlaying && !isPaused) {
      setVisValues([10, 10, 10, 10, 10, 10, 10, 10])
      return
    }
    const update = () => {
      const Tone = toneRef.current
      if (Tone) {
        ...
        // Generate responsive visualizer values
        setVisValues(Array.from({ length: 8 }, () => Math.floor(Math.random() * 80) + 20))
      }
      ...
      {items.map((pad) => {
        ...
        return (
          <m.button
            ...
            className={cn(
              'group relative flex flex-col items-center justify-center overflow-hidden rounded-md border border-zinc-800 border-b-4 border-b-zinc-950 bg-zinc-900 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_4px_10px_rgba(0,0,0,0.5)] transition-all duration-100 active:translate-y-0.5 active:border-b-2 sm:rounded-lg',
              pad.type === 'functional' ? 'z-10' : 'z-0',
              isLooping || isActive ? 'border-t-white/20' : ''
            )}
            style={{ gridColumn: `span ${pad.w}`, gridRow: `span ${pad.h}` }}
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
          >
            {/* Glow — persistent when looping, brief on active, soft on hover */}
            <div className={cn(
              'absolute inset-0 z-0 transition-all duration-200', 
              pad.color, 
              isLooping 
                ? 'opacity-85 shadow-[inset_0_0_15px_rgba(255,255,255,0.3),0_0_20px_currentColor]' 
                : isActive 
                  ? 'opacity-100' 
                  : 'opacity-0 group-hover:opacity-40 group-hover:blur-xs'
            )} />

            {/* Content */}
            {pad.type === 'functional' && 'icon' in pad && (
              <div className="relative z-10 flex flex-col items-center gap-1 sm:gap-2">
                <pad.icon className={cn('h-5 w-5 transition-all duration-200 sm:h-8 sm:w-8', isLooping || isActive ? 'text-white scale-110 drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]' : 'text-zinc-500')} />
                <div className="hidden flex-col items-center sm:flex">
                  <span className={cn('text-[10px] font-bold tracking-wider transition-colors duration-200 sm:text-xs', isLooping || isActive ? 'text-white font-extrabold' : 'text-zinc-400')}>
                    {(pad as any).label}
                  </span>
                  {(pad as any).subLabel && (
                    <span className={cn('font-mono text-[8px] transition-colors duration-200 sm:text-[10px]', isLooping || isActive ? 'text-white/80' : 'text-zinc-650')}>
                      {(pad as any).subLabel}
                    </span>
                  )}
                </div>
            ...
            <div className={cn(
              'absolute top-1 right-1 h-1.5 w-1.5 rounded-full transition-all duration-200 sm:top-2 sm:right-2', 
              isLooping 
                ? 'animate-pulse bg-white shadow-[0_0_8px_white]' 
                : isActive 
                  ? 'bg-white' 
                  : 'bg-zinc-950/80 shadow-[inset_0_1px_1px_rgba(0,0,0,0.8)]'
            )} />

            {/* Loop indicator */}
            {isLooping && (
              <div className="absolute bottom-1 left-1 h-1 w-1 rounded-full bg-white/60 sm:bottom-2 sm:left-2" />
            )}
          </m.button>
        )
      })}
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="relative mx-auto max-w-6xl rounded-3xl bg-zinc-800 p-2 shadow-2xl sm:p-4 dark:bg-zinc-950">
            ...
            <div className="relative rounded-2xl border border-zinc-700 bg-zinc-900 p-4 shadow-inner sm:p-6 md:p-10">
              ...
              {/* Top Panel with Visualizer */}
              <div className="mb-6 flex flex-col gap-4 border-b border-zinc-800 pb-4 md:flex-row md:items-center md:justify-between px-2">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-2">
                    <div className={cn('h-2 w-2 rounded-full', isPlaying ? 'animate-pulse bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]' : 'bg-zinc-600')} />
                    <span className="font-mono text-[10px] font-bold tracking-widest text-zinc-500 sm:text-xs">LIVE LED REC</span>
                  </div>
                  <span className="text-[11px] font-black tracking-[0.4em] text-zinc-400">LAUNCHPAD PRO V2</span>
                </div>

                {/* Digital Spectrum Visualizer Display */}
                <div className="flex items-center gap-3 rounded-lg border border-zinc-800/80 bg-black/40 p-2 px-3 shadow-inner">
                  <span className="font-mono text-[8px] font-bold text-zinc-500 uppercase tracking-wider">Spectrum</span>
                  <div className="flex h-5 items-end gap-[2px] w-28 px-1 border-r border-zinc-800/60 mr-1">
                    {visValues.map((val, idx) => (
                      <div
                        key={idx}
                        className="w-1.5 rounded-[1px] bg-emerald-500 transition-all duration-75"
                        style={{
                          height: `${val}%`,
                          backgroundColor: idx > 5 ? '#f43f5e' : idx > 3 ? '#eab308' : '#10b981',
                  ...
                  <div className="flex items-center gap-2 font-mono text-[10px]">
                    <span className="text-emerald-400/90">{transportTime}</span>
                    <span className="text-zinc-600">|</span>
                    <span className="text-amber-400/90">{bpm} BPM</span>
                  </div>
                </div>
              </div>
