    ...
    <section ref={sectionRef} id="skills" className="overflow-hidden py-24">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <Screw className="absolute bottom-8 left-8 transition-transform hover:-rotate-12" />
          <Screw className="absolute right-8 bottom-8 transition-transform hover:rotate-12" />
          ...
          >
            ...
            <div className="mb-8 flex flex-col gap-6 border-b border-black/10 pb-6 md:flex-row md:items-center md:justify-between dark:border-white/5">
              {/* Device Branding */}
              <div>
                <h3 className="text-lg font-black tracking-widest text-black/60 uppercase dark:text-white/60">
                  MIDI-CONTROL
                </h3>
                <p className="mt-0.5 font-mono text-[9px] tracking-widest text-black/40 uppercase dark:text-white/30">
                  Interactive Skills & Synthesizer Interface
                </p>
              </div>

              {/* Top Panel Controls: LCD, VU Meter, Power Switch */}
              <div className="flex w-full flex-wrap items-center justify-between gap-4 md:w-auto">
                {/* Horizontal VU Meter */}
                ...
                <div className="flex h-12 w-48 shrink-0 flex-col justify-center rounded-[3px] border border-black/40 bg-[#0A0A0C] px-3.5 py-1.5 font-mono text-[9px] text-emerald-400 shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)]">
                  <div className="flex justify-between text-[7px] font-bold tracking-wider text-emerald-600">
                    <span>CHANNEL: 01</span>
                    <span>OCTAVE: 0</span>
                  </div>
                  <div className="text-shadow-glow mt-0.5 truncate font-black tracking-widest text-emerald-300 uppercase">
                  ...
                  <button
                    onClick={() => {
                      if (isOn) {
                        ...
                        setActiveMessage('SYSTEM ON: BOOTING...')
                        setTimeout(
                          () => setActiveMessage('MIDI KEYBOARD READY'),
                          800,
                        )
                      }
                    }}
                    ...
                    className={cn(
                      'relative flex h-9 w-6 cursor-pointer flex-col justify-between rounded border border-black bg-zinc-950 p-[1px] shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)] outline-none',
                    )}
                  >
                    {/* Toggle slider cap */}
                    <div
                      className={cn(
                        'flex h-4 w-full items-center justify-center rounded-[1px] border border-black/30 bg-gradient-to-b from-[#B8B9C4] to-[#7B7C84] shadow-md transition-all duration-200',
            ...
            <div className="mb-8 grid grid-cols-1 items-stretch gap-8 lg:grid-cols-12 lg:gap-8">
              {/* Group 1: velocity Drum Pads (Languages) */}
              <div className="rounded-md border border-black/15 bg-[var(--daw-chassis-mid)] p-4 shadow-inner lg:col-span-5 dark:border-white/5">
                <div className="mb-4 flex items-center justify-between">
                  <h4 className="font-mono text-[8px] font-black tracking-wider text-black/40 uppercase dark:text-white/30">
              ...
              <div className="rounded-md border border-black/15 bg-[var(--daw-chassis-mid)] p-4 shadow-inner lg:col-span-4 dark:border-white/5">
                <div className="mb-4 flex items-center justify-between">
                  <h4 className="font-mono text-[8px] font-black tracking-wider text-black/40 uppercase dark:text-white/30">
                ...
                <div className="grid h-36 grid-cols-2 items-center gap-4">
              ...
              <div className="rounded-md border border-black/15 bg-[var(--daw-chassis-mid)] p-4 shadow-inner lg:col-span-3 dark:border-white/5">
                <div className="mb-4 flex items-center justify-between">
                  <h4 className="font-mono text-[8px] font-black tracking-wider text-black/40 uppercase dark:text-white/30">
                    FADERS C: TOOLS
                  </h4>
            ...
            <div className="flex items-end gap-4 overflow-hidden border-t border-black/15 pt-6 select-none dark:border-white/5">
              {/* Pitch Bend / Mod Wheels */}
              <div className="hidden shrink-0 items-end gap-2.5 pb-2 sm:flex">
                <MidiWheel
                  label="PITCH"
                  isOn={isOn}
                  onChange={(v) =>
                    setActiveMessage(`PITCH WHEEL: ${v > 0 ? '+' : ''}${v}`)
                  }
                />
                <MidiWheel
                  label="MOD"
              ...
              <div className="no-scrollbar flex-1 overflow-x-auto rounded-md border border-black/30 shadow-lg">
                {/* LED indicator line directly above keys */}
                <div className="relative flex h-2.5 w-full border-b border-black bg-zinc-950 px-[1px]">
                  {PIANO_KEYS.map((key, i) => {
                    const isScanning =
                      bootScanIndex !== null && bootScanIndex === i
                    const isActive =
                      (isOn && activeKey === key.note) || isScanning
                    return (
                      <div
                        key={key.note}
                ...
                <div
                  className={cn(
                    'relative flex w-full min-w-[560px] bg-zinc-900 py-0.5 transition-opacity duration-300',
                    !isOn && 'pointer-events-none opacity-60',
                  )}
                >
                  {/* White keys */}
                  {PIANO_KEYS.filter((k) => !k.isBlack).map((key) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={!isOn}
                        className={cn(
                          'h-32 flex-1 rounded-b-[4px] border-x border-t border-b-[3px] border-black/35 transition-all outline-none select-none',
                          isActive
                            ? 'border-b-2 border-zinc-400 bg-zinc-200 pt-2 shadow-inner'
                            : 'border-zinc-300 bg-white shadow-[inset_0_2px_4px_rgba(255,255,255,1),0_2px_4px_rgba(0,0,0,0.15)] hover:bg-zinc-50',
                        )}
                      />
                    )
                  })}
                  ...
                  {PIANO_KEYS.filter((k) => k.isBlack).map((key) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={!isOn}
                        className={cn(
                          'absolute z-20 h-20 w-3 rounded-b-[3px] border border-black bg-gradient-to-b from-zinc-800 to-black shadow-[0_2px_4px_rgba(0,0,0,0.4)] transition-all outline-none select-none',
                          isActive
                            ? 'scale-y-95 border-zinc-700 from-zinc-900 to-zinc-950 shadow-inner'
...
function DrumPad({
  ...
  return (
    <button
      ...
      disabled={!isOn}
      className={cn(
        'relative flex h-[68px] w-20 flex-col justify-between rounded-md border p-2 transition-all outline-none select-none',
        isOn ? 'cursor-pointer' : 'cursor-not-allowed opacity-30',
        isLit
          ? 'scale-95 bg-zinc-800'
          : 'border-zinc-950/80 bg-zinc-900 shadow-[0_3px_5px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.05)]',
      )}
      style={{
        boxShadow: isLit ? `0 0 14px ${color}, inset 0 0 8px ${color}` : 'none',
        borderColor: isLit ? '#ffffff' : isOn ? `${color}40` : 'transparent',
      }}
    >
      <div className="flex w-full items-center justify-between">
        <span className="font-mono text-[5px] font-bold text-white/30 uppercase">
          PAD {index + 1}
        </span>
        <div
      ...
      <span className="w-full truncate text-center font-mono text-[7px] font-black tracking-wider text-white uppercase">
        {name}
      </span>
      <span className="w-full text-center font-mono text-[6px] font-bold text-white/50">
        {`${level}%`}
      </span>
    </button>
  )
...
function FrameworkKnob({
  name,
  level,
  index,
  onChange,
  isOn,
}: FrameworkKnobProps & { isOn: boolean }) {
  ...
  return (
    <div
      className={cn(
        'flex flex-col items-center gap-1 transition-opacity duration-300 select-none',
        !isOn && 'opacity-35',
      )}
    >
      <motion.div
        ...
        className={cn(
          'relative flex h-12 w-12 items-center justify-center rounded-full border-2 border-black/40 bg-gradient-to-br from-zinc-700 via-zinc-800 to-zinc-950 shadow-md',
      ...
      <span className="mt-1 w-full truncate text-center font-mono text-[7px] font-bold tracking-wider text-black/50 uppercase dark:text-white/40">
        {name}
      </span>
      <span className="mt-0.5 font-mono text-[6px] font-bold text-black/30 dark:text-white/20">
        {val}%
      </span>
    </div>
  )
...
function ToolsFader({
  ...
  return (
    <div
      className={cn(
        'flex h-36 flex-1 flex-col items-center gap-1 py-1 transition-opacity duration-300 select-none',
        !isOn && 'opacity-35',
      )}
    >
      <div
        ref={railRef}
        className={cn(
          'relative flex w-4 flex-1 justify-center',
      ...
      >
        {/* Recessed track slot */}
        <div className="absolute top-0 bottom-0 w-1 rounded-full border border-black/10 bg-zinc-950 shadow-[inset_0_1.5px_3px_rgba(0,0,0,0.85)]" />
        ...
        <motion.div
          ...
          transition={{ type: 'spring', stiffness: 90, damping: 14 }}
          className="pointer-events-none absolute flex h-5 w-6 flex-col items-center justify-center gap-[2px] rounded-[2px] border border-black/45 bg-gradient-to-b from-[#B8B9C4] to-[#6B6C74] shadow-md"
          style={{ transform: 'translateY(-50%)' }}
        >
          <div className="h-[1px] w-4 bg-black/30" />
          {isOn && (
            <div
  ...
  return (
    ...
    >
      <div
        ref={trackRef}
        className={cn(
          'relative h-28 w-6 overflow-hidden rounded-[3px] border border-black/40 bg-zinc-950 shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)]',
          isOn ? 'cursor-ns-resize' : 'cursor-not-allowed',
        )}
        onPointerDown={handlePointerDown}
        onPointerMove={handlePointerMove}
      ...
      >
        ...
        <motion.div
          ...
          className="absolute right-0.5 left-0.5 h-7 rounded-[1px] border border-black/45 bg-gradient-to-b from-[#B8B9C4] via-[#7B7C84] to-[#4B4C52] shadow-md"
          style={{ transform: 'translateY(-50%)' }}
        >
          <div className="absolute top-1/2 right-0.5 left-0.5 h-px bg-black/40" />
          <div className="absolute top-1/4 right-0.5 left-0.5 h-px bg-white/20" />
          <div className="absolute right-0.5 bottom-1/4 left-0.5 h-px bg-black/20" />
          {/* Internal LED indicator */}
          {isOn && (
            <div className="absolute top-1/2 left-1/2 h-[3px] w-2 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#C9A447] shadow-[0_0_3px_#C9A447]" />
          )}
        </motion.div>
      </div>
      <span className="mt-0.5 font-mono text-[5px] font-black tracking-widest text-black/45 uppercase dark:text-white/30">
        {label}
      </span>
    </div>
  )
...
function HorizontalVUMeter({ isOn }: { isOn: boolean }) {
  return (
    <div className="flex w-44 flex-col justify-center gap-1.5 rounded-[3px] border border-black/40 bg-[#0A0A0C] p-2 shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)]">
      {[...Array(2)].map((_, ch) => (
        <div key={ch} className="flex w-full items-center gap-[1.5px]">
          {[...Array(15)].map((_, i) => {
            const isRed = i >= 13
...
function VUSegment({
  ...
  return (
    <div
      className={cn(
        'h-3 w-[7px] rounded-[1px] transition-all duration-75',
        color,
      )}
      style={{
        opacity: opacity,
