...
export function Footer() {
  ...
  return (
    <footer
      ref={ref}
      className="relative overflow-hidden bg-zinc-950 py-16 border-t border-zinc-800 text-zinc-300"
    >
      ...
      <Screw className="absolute top-4 left-4 opacity-50" />
      <Screw className="absolute top-4 right-4 opacity-50" />
      <Screw className="absolute bottom-4 left-4 opacity-50" />
      <Screw className="absolute right-4 bottom-4 opacity-50" />

      {/* Standard Centered Container */}
      <div className="relative container mx-auto px-4 md:px-6 flex flex-col gap-8">
        
        {/* Top Board Rails */}
        <div className="flex items-center justify-between border-b border-zinc-900 pb-4 font-mono text-[9px] text-zinc-500 tracking-[0.2em] uppercase">
          <span>DAW OUTPUT STAGE : SYSTEM CONTROL</span>
          <div className="flex gap-2">
            <span className="text-zinc-700">•</span>
            <span>STEREO MONITORS</span>
            <span className="text-zinc-700">•</span>
            <span className="text-green-500 animate-pulse font-bold">READY</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 items-center">
          ...
          <div className="lg:col-span-4 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded bg-zinc-900 border border-zinc-850 shadow-md">
                <Radio className={`h-5 w-5 ${isPlaying ? 'text-primary animate-pulse' : 'text-zinc-650'}`} />
              </div>
              <div className="min-w-0">
                <h3 className="truncate font-mono text-sm font-bold text-zinc-200 tracking-wider">
                  ADITYAHIMAONE.MIX
                </h3>
            ...
            <div className="h-10 flex items-end gap-[2px] bg-zinc-900/40 rounded border border-zinc-900/80 px-3 py-1.5 overflow-hidden">
              {[...Array(32)].map((_, i) => {
          ...
          <div className="lg:col-span-4 flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-4 rounded-full bg-zinc-900/60 border border-zinc-850 p-2 shadow-inner">
              <button
                onClick={() => setIsPlaying(false)}
                className="p-2 rounded-full text-zinc-500 hover:text-zinc-300 transition-colors hover:bg-zinc-850"
                title="Stop"
              >
              ...
              <div className="px-2 flex items-center gap-1.5">
                <Volume2 className="h-4 w-4 text-zinc-650" />
                <span className="font-mono text-[9px] text-zinc-500 font-bold">0dB</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {SOCIAL_LINKS.map((link) => (
                <a
                  ...
                  className="flex items-center justify-between rounded border border-zinc-900 bg-zinc-900/30 p-2.5 hover:bg-zinc-900/80 hover:border-zinc-800 transition-all font-mono"
                >
                  ...
                  {/* Glowing LED Socket */}
                  <div className="h-4 w-4 rounded-full bg-zinc-950 border border-zinc-900 flex items-center justify-center shrink-0">
                    <div className={`h-2 w-2 rounded-full transition-all duration-300 ${isPlaying ? 'bg-green-500/80 shadow-[0_0_6px_rgba(34,197,94,0.8)]' : 'bg-zinc-850'}`} />
                  </div>
                </a>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 border-t border-zinc-900 pt-6">
          ...
          <div className="flex flex-col gap-2 md:col-span-2 text-left">
            ...
            <div className="flex flex-wrap gap-1.5">
              {TECH_STACK.map((tech) => (
                <span
                  key={tech}
                  className="rounded bg-zinc-900 border border-zinc-850 px-2 py-0.5 font-mono text-[9px] font-bold text-zinc-500 uppercase tracking-tight"
                >
                  {tech}
                </span>
              ))}
        ...
        {/* Bottom Metatags Bar */}
        <div className="flex flex-col sm:flex-row items-center justify-between border-t border-zinc-900 pt-6 text-[10px] text-zinc-550 font-mono gap-4">
          <span>
